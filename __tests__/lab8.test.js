describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    await button.click();
    expect(await (await button.getProperty('innerText')).jsonValue()).toBe("Remove from Cart");
  }, 2500);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartElement = await page.$('#cart-count');
    expect(await (await cartElement.getProperty('innerText')).jsonValue()).toBe("20");
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cartElement = await page.$('#cart-count');
    expect(await (await cartElement.getProperty('innerText')).jsonValue()).toBe("0");
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      let shadowRoot = await prodItems[i].getProperty('shadowRoot');
      let button = await shadowRoot.$('button');
      expect(await (await button.getProperty('innerText')).jsonValue()).toBe("Add to Cart");
    }
